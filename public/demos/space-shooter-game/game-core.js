...
export function startGame(selectedSkin){
  ...
  const state = { score:0, lives:3, level:1, wave:1, enemiesLeft:0, gameState:'playing', powerup:null, powerupTimer:0, paused:false,
    spawning:false, wavesCompleted:0, enemiesSpawnedInWave:0 };
  ...
  function spawnBoss(){ if(!bossModel) return; const cfg=getLevelConfig(state.level); boss=bossModel.clone(); boss.position.set(0,0,-gameHeight/2-12); boss.health=cfg.bossHealth; boss.maxHealth=boss.health; bossHud.style.display='block'; updateBossHUD(); scene.add(boss); }
  ...
  function restart(){ enemies.forEach(o=>scene.remove(o)); bullets.forEach(o=>scene.remove(o)); particles.forEach(o=>scene.remove(o)); powerups.forEach(o=>scene.remove(o)); if(boss){scene.remove(boss); boss=null;} enemies.length=bullets.length=particles.length=powerups.length=0;
    state.score=0; state.lives=3; state.level=1; state.wave=1; state.gameState='playing'; state.enemiesLeft=0; state.powerup=null; state.powerupTimer=0; state.paused=false; state.spawning=false; state.wavesCompleted=0; state.enemiesSpawnedInWave=0; updateHUD(); bossHud.style.display='none'; if(uiPowerup) uiPowerup.style.display='none'; if(player) player.position.set(0,0,5); startLevel(); if(sndMusic.buffer){ if(!sndMusic.isPlaying){ sndMusic.setVolume(0); sndMusic.play(); fadeAudio(sndMusic,0.25,1200);} else { sndMusic.setVolume(.25);} } }
